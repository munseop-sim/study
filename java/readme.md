### 1. JVM구조 설명
- 클래스로더 
  - 자바 클래스 파일을 로드하여 JVM으로 전달
- 런타임(메모리)영역: 
  - STACK: 스레드별로 생성되며 메소드 호출시에 생성되는 지역변수와 메소드 호출정보를 저장
  - HEAP: 객체와 배열같은 동적으로 생성된 데이터가 저장되는 영역, GC대상
  - PC: 각 스레드의 현재 실행 중인 JVM 명령어 주소를 저장하는 영역
  - 네이티브 메서드 스택
- 실행엔진: 클래스파일로 변환된 자바코드를 읽고 실행
  - 인터프리터
  - JIT
  - GC
